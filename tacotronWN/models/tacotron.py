...
class Tacotron():
  ...
  def initialize(self, inputs, input_lengths, mel_targets=None, reference_mel = None, stop_token_targets = None, linear_targets=None, targets_lengths=None, gta=False,
    ...
    with tf.variable_scope('inference') as scope:
      ...
      (decoder_outputs, _), final_decoder_state, _ = tf.contrib.seq2seq.dynamic_decode(
        BasicDecoder(output_cell, helper, decoder_init_state), impute_finished=False,
  ...
  def add_loss(self):
    ...
    with tf.variable_scope('loss') as scope:
      ...
      if hp.mask_decoder:
        ...
        before = MaskedMSE(self.mel_targets, self.mel_outputs, self.targets_lengths)
        # Compute loss after postnet
        after = MaskedMSE(self.linear_targets, self.linear_outputs, self.targets_lengths)
        #Compute <stop_token> loss (for learning dynamic generation stop)
        #stop_token_loss = MaskedSigmoidCrossEntropy(self.stop_token_targets,self.stop_token_prediction, self.targets_lengths)
      else:
        # Compute loss of predictions before postnet
        before = tf.losses.mean_squared_error(self.mel_targets, self.mel_outputs)
        # Compute loss after postnet
        after = tf.losses.mean_squared_error(self.linear_targets, self.linear_outputs)
        #Compute <stop_token> loss (for learning dynamic generation stop)
        #stop_token_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.stop_token_targets, logits=self.stop_token_prediction))
      ...
      if hp.gst_scale_regularization:
        reg_weight_scaler = 1. / (2 * hp.max_abs_value) if hp.symmetric_mels else 1. / (hp.max_abs_value)
        reg_weight = hp.gst_reg_weight * reg_weight_scaler
      else:
        reg_weight = hp.gst_reg_weight
      ...
      self.loss = self.before_loss + self.after_loss  + self.mel_loss + self.regularization_loss + self.linear_loss
