...
import threading
import os
from .wave_util import is_scalar_input, is_mulaw_quantize
		...
		if local_condition:
			...
			for b in batch:
				...
				if max_time_steps is not None:
					max_steps = _ensure_divisible(max_time_steps, audio.get_hop_size(), True)
					if len(x) > max_time_steps:
						max_time_frames = max_steps // audio.get_hop_size()
						start = np.random.randint(0, len(c) - max_time_frames)
						time_start = start * audio.get_hop_size()
